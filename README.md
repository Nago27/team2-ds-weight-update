...
private:
   ...
   vector<f32> data_;
public:
   inline f32 operator()(i32 r, i32 c) const { 
...
void Forward(const Matrix& X, Matrix& out) {
   ...
   for (size_t i = 0; i < layers_.size(); ++i) {
      layers_[i]->Forward(cur, nxt); acts_[i + 1] = nxt; cur = acts_[i + 1];
   }
   out = acts_.back();
}
void Backward(const Matrix& dOut) {
   ...
   for (int i = static_cast<int>(layers_.size()) - 1; i >= 0; --i) {
      layers_[i]->Backward(acts_[i], cur_d, prev_d); cur_d = prev_d;
   }
}
```
- 불필요한 연산 dX
...
2. 루프 구조 최적화 (병렬처리) <br>: 레이어마다 행렬의 크기가 다르다는 것을 고려하여 각 함수의 내부에서도 if문으로 분기를 만들어 총 5가지의 루프를 구현하였습니다.<br>각 루프의 순서는 OpenMP(스레드 병렬)/AVX2(SIMD 병렬)를 활용한 병렬화 효율과 스레드 간의 Race Condition을 고려하여 결정하였습니다.
3. Row-major 연속 접근 유지로 캐시 효율 극대화<br>: 루프 순서를 바꾸는 과정에서 열 단위 접근이 발생하는 행렬은 함수 내부에서 전치 행렬을 생성하여 연산에 활용함으로써, 비효율적인 열 단위 접근을 행 단위 접근으로 변환하였습니다.<br>결과적으로 루프의 순서가 바뀌더라도 Row_major로 저장된 연속적인 자료구조를 순차적으로 읽을 수 있게 되어 메모리의 연속성을 극대화하고 캐시 미스를 최소화하였습니다.
4. 희소성 활용<br>: 연산 과정에서 데이터가 0인 경우(ReLU를 통해 변환된 데이터가 0으로 바뀌는 것과 기존 데이터 중 0값도 포함) 연산을 생략(```continue```)하는 방식으로 데이터의 희소성을 활용하였습니다.
5. 기타 연산 최적화<br>: ```AddRowBias```나 ```ReLUForward``` 등 다른 모든 행렬 연산에서도 행 단위 접근을 극대화하고 OpenMP, AVX2를 적절히 사용하였습니다.
6. OpenMP & AVX2 적용 방법
	- C/C++ > 언어 > OpenMP 지원 > 예(/openmp)
	- C/C++ > 코드 생성 > 고급 명령 집합 사용 > 고급 벡터 확장 2(X86/X64)(/arxh:AVX2)
	- ```#include <immintrin.h>``` 추가
<div style="page-break-after: always;"></div>
...
2. Train과 SliceBatch 메모리 처리 효율화 (memcpy 및 병렬화 적용)
...
- After
<img width="500" height="400" alt="Image" src="https://github.com/user-attachments/assets/7e366b91-7e33-4f6a-9e67-fefcac4804e0" />
...
- After
<img width="500" height="400" alt="Image" src="https://github.com/user-attachments/assets/742b4b2f-e863-4ed1-bfcf-ea0187835bf9" />
...
- After
<img width="500" height="400" alt="Image" src="https://github.com/user-attachments/assets/11c9f10d-f802-4a62-90f5-fb6888d1e247" />
...
- After
<img width="500" height="400" alt="Image" src="https://github.com/user-attachments/assets/b841c0bb-4096-4f22-abc0-28a0d7716b22" />
