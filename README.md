...
시간을 가장 많이 차지하는 행렬곱 연산을 Row_major 방식으로 저장된 자료구조의 특성을 가장 잘 활용할 수 있는 구조로 바꾸었다. 먼저 행렬곱 연산마다 행렬의 전치 형태($A \times B$, $A^T \times B$ 등)가 다르다는 점을 고려하여 MatMul 함수를 3가지로 분리하였다. 또한 각 함수의 내부에서도 레이어마다 행렬의 크기가 다르다는 것을 고려하여 if문으로 분기를 만들어 총 5가지의 루프를 구현하였다. 각 루프의 순서는 스레드 간의 경쟁 상태와 OpenMP/AVX2를 활용한 병렬화 효율을 고려하여 결정하였다. 루프 순서를 바꾸는 과정에서 열 단위 접근이 발생하는 행렬은 함수 내부에서 전치 행렬을 생성하여 연산에 활용함으로써 비효율적인 열 단위 접근을 행 단위 접근으로 변환하였다. 결과적으로 루프의 순서가 바뀌더라도 Row_major로 저장된 연속적인 자료구조를 순차적으로 읽을 수 있게 되어 메모리의 연속성을 극대화하고 캐시 미스를 최소화하였다.
추가적으로 연산 과정에서 입력 데이터가 0인 경우 연산을 생략하는 방식으로데이터의 희소성도 활용하였다.
AddRowBias나 ReLUForward등 다른모든 행렬 연산에서도 행 단위 접근을 극대화하고 OpenMP, AVX2를 적절히 사용하였다.
