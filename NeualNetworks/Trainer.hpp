// =============================
// include/vsnn/Trainer.hpp (최종 최적화 1/4)
// (가장 큰 병목이었던 Matrix 전체 복사 제거)
// =============================
#pragma once
#include <vector>
#include <algorithm>
...
namespace vsnn {
	...
	class Trainer {
	private:
		// [최적화] 셔플된 인덱스(idx)를 받아 원본 X에서 직접 데이터를 가져오도록 변경
		static void SliceBatch(const Matrix& X, const vector<int>& y, const vector<int>& idx,
			int beg, int end, Matrix& Xb, vector<int>& yb) {
			const int N = end - beg; const int D = X.Cols();
			if (Xb.Rows() != N || Xb.Cols() != D) Xb.Reset(N, D);
			yb.resize(N);
			for (int i = 0; i < N; ++i) {
				int original_index = idx[beg + i]; // 셔플된 인덱스
				for (int d = 0; d < D; ++d) {
					Xb(i, d) = X(original_index, d); // 원본 X에서 데이터 가져오기
				}
	...
	public:
		...
		static TrainReport Train(Sequential& model, const Matrix& X, const vector<int>& y, const TrainConfig& cfg) {
			...
			for (int r = 0; r < cfg.repeats; ++r) {
				...
				// [최적화] 훈련 데이터(X) 전체를 복사하던 (Matrix Xs) 로직 *완전 삭제*
				// (이것이 가장 큰 병목이었음)
			...
			auto median_of = [](vector<double>& v) {
